...
int main(void) {
	...
	init_RFID125khz();
//    init_configRFID(bitrate_RF_64, modulationRF_MANCH, pskCF_RF_2); 
    while (1) {
 ...
 //           ebus_txfifo_push(buf, 48, 1);
            read_tag(buf);
            buf[13] = 88;	// кол-во бит данных в пакете (команда, код девайса, 8 байт считанный ключ и один байт тип модуляции)
            ebus_txfifo_push(buf, 112, 1);
            heartbeat_count = req_life;
        }
