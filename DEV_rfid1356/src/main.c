...
//            ebus_txfifo_push(buf, 48, 1);
            read_tag(buf);
            buf[22] = 160;	// кол-во бит данных в пакете (команда, код девайса, данные структуры cur_NFCteg полученные при чтении метки)
            ebus_txfifo_push(buf, 184, 1);
            heartbeat_count = req_life;
        }
