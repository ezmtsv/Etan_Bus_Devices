...
*/
    GPIOA->MODER = GPIO_OUT(PIN_LED_G) | GPIO_INP(RX_PIN) | GPIO_OUT(LINEOK_PIN) | GPIO_OUT(FAN_PIN) | GPIO_OUT(LOAD_PIN);
...
/*
	GPIOA->PUPDR = GPIO_PUP(PIN_BTN0) | GPIO_PUP(PIN_BTN1) |
                   ...
                   GPIO_PUP(LINEOK_PIN);
*/
		GPIOA->PUPDR = GPIO_PUP(LINEOK_PIN);
    GPIOA->AFR[0] = 0x00U;
    GPIOA->AFR[1] = 0x00U;
}
...
void Load_on(void) {
    GPIOA->BSRR = _BV(LOAD_PIN);
}

void Load_off(void) {
    GPIOA->BRR = _BV(LOAD_PIN);
}

void FAN_on(void) {
    GPIOA->BSRR = _BV(FAN_PIN);
}

void FAN_off(void) {
    GPIOA->BRR = _BV(FAN_PIN);
}

void tooglePIN_G(void) {
		GPIOA->ODR ^= (1<<PIN_LED_G);
}
...
uint8_t device_state(void) {
    uint8_t res = 0;
	/*
    res = (GPIOA->IDR & _BV(PIN_BTN0)) ? 0x01U : 0x00U;
    res |= (GPIOA->IDR & _BV(PIN_BTN1)) ? 0x02U : 0x00U;
    res |= (GPIOA->IDR & _BV(PIN_BTN2)) ? 0x04U : 0x00U;
    res |= (GPIOA->IDR & _BV(PIN_BTN3)) ? 0x08U : 0x00U;
	*/
    return res;
}
